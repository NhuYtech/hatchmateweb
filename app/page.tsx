...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async (e: React.FormEvent) => {
    ...
    if (!isFirebaseConfigured) {
      setErrorMsg("Firebase chưa được cấu hình. Vui lòng sử dụng chế độ Đăng nhập Thử nghiệm.");
      return;
    }
  ...
  const handleDemoLogin = (e: React.FormEvent) => {
    ...
    // Simulate a brief delay to make the transition look professional
    setTimeout(() => {
      signInMock();
      router.replace("/dashboard");
    }, 800);
      ...
      <div className="relative w-full max-w-[460px] mx-4 bg-white/95 rounded-[32px] shadow-[0_24px_70px_rgba(224,140,0,0.22)] border border-white/60 p-8 sm:p-10 md:p-12 z-10 backdrop-blur-md transition-all duration-500 hover:shadow-[0_30px_80px_rgba(224,140,0,0.3)] hover:scale-[1.01] flex flex-col items-center">
        ...
        <div className="w-full flex flex-col items-center">
          <button
            ...
            className={`group relative w-full flex items-center justify-center gap-3.5 px-6 py-4 bg-white border border-gray-150 rounded-full shadow-[0_5px_15px_rgba(0,0,0,0.04)] text-gray-700 font-bold text-sm sm:text-[15px] select-none transition-all duration-300 ${
              isLoading || !isFirebaseConfigured
              ? "bg-gray-50 border-gray-200 text-gray-400 cursor-not-allowed opacity-60"
              : "hover:shadow-[0_8px_25px_rgba(245,176,0,0.18)] hover:border-[#FFD56B]/60 hover:-translate-y-[2.5px] active:translate-y-0 cursor-pointer"
            }`}
          >
            {isLoading ? (
              <>
                <svg className="animate-spin h-5 w-5 text-[#F5B000]" fill="none" viewBox="0 0 24 24">
                  <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                  <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                </svg>
                <span className="font-semibold text-gray-500">Đang kết nối...</span>
              </>
            ) : (
              <>
                {/* Official Google G Icon */}
                <svg className="w-5 h-5 shrink-0 transition-transform duration-300 group-hover:scale-105" viewBox="0 0 24 24">
                  <path
                    fill="#4285F4"
          ...
          {/* Dedicated Mock Login Button (Demo Mode) */}
          {(!isFirebaseConfigured || process.env.NODE_ENV === "development") && (
            <button
              onClick={handleDemoLogin}
              disabled={isDemoLoading}
              className={`mt-4 group relative w-full flex items-center justify-center gap-3 px-6 py-4 bg-gradient-to-r from-amber-500 to-[#F5B000] text-white rounded-full shadow-[0_5px_20px_rgba(245,176,0,0.25)] font-bold text-sm sm:text-[15px] select-none transition-all duration-300 ${
                isDemoLoading
                ? "opacity-85 cursor-not-allowed"
                : "hover:shadow-[0_8px_25px_rgba(245,176,0,0.4)] hover:-translate-y-[2px] active:translate-y-0 cursor-pointer hover:brightness-105"
              }`}
            >
              {isDemoLoading ? (
                <>
                  ...
                  <span className="font-semibold text-white">Đang đăng nhập...</span>
                </>
              ) : (
                <>
                  <span>Vào chế độ Thử nghiệm (Demo)</span>
                </>
              )}
            </button>
          )}
