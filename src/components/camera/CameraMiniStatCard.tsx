...
interface CameraMiniStatCardProps {
  label: string;
  value: number | string;
  icon: LucideIcon;
...
export default function CameraMiniStatCard({
  label,
  value,
  icon: Icon,
  accent,
}: CameraMiniStatCardProps) {
  const accentClasses = {
    sky: {
      bg: "bg-sky-50",
      text: "text-sky-600",
  ...
  return (
    <div
      className={`group flex items-center justify-between rounded-[24px] border bg-white p-5 shadow-sm shadow-sky-100/10 transition-all duration-300 relative overflow-hidden ${activeStyles.border} ${activeStyles.hover} hover:-translate-y-1 hover:shadow-lg`}
    >
      {accent === "indigo" && (
        <div
